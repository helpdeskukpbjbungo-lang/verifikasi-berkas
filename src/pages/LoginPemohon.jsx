...
export default function LoginPemohon() {
  ...
  if (user) {
    return <Navigate to="/dashboard/pemohon" replace />
  }
  ...
  const handleSubmit = async (e) => {
    ...
    const { data, error: err } = await signIn(email, password, 'admin_pemohon')
    ...
    <div className="min-h-screen flex items-center justify-center bg-background">
      <div className="w-full max-w-md p-8 bg-surface rounded-xl shadow-lg border border-outline-variant">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-primary">Login Pemohon</h1>
          <p className="text-on-surface-variant mt-2">Masuk sebagai admin pemohon</p>
        </div>
